// "Start" landing page introducing the four media search engines
export function render(appEl) {
  appEl.innerHTML = `
      <section class="start">
        <h2>Explore metadata across four sources</h2>
        <h3>Search a plethora of variables and parameters depending on filetype.</h3>
        To view more detailed information about a specific document, you can see the raw metadata.
        <br><br>

        <!-- Navigation cards linking to each engine via hash routes -->
        ...
        <!-- Quick tips for users exploring the UI -->
        <div class="tips">
          <br>
          <strong>Tips:</strong>
          <ul>
...
export function cleanup() { /* No-op for now */ }
