...
export default function setupPptRestRoutes(app, db) {
  ...
  router.get('/api/ppt', requireDb, async (req, res) => {
    ...
    try {
      ...
      res.json({ items: rows, limit, total });
    } catch (err) {
  ...
  router.get('/api/ppt-search/:field/:searchValue', requireDb, async (req, res) => {
    ...
    // placeholder '-' eller tom = ingen sökterm
    ...
    try {
      ...
      if (field === 'any') {
        ...
        if (field === 'creationDate') {
          // ignorerar searchValue, endast datumintervall används
        } else if (searchValue) {
  ...
  FROM powerpoint_metadata
  ${whereClause}
  LIMIT ${limit} OFFSET ${offset}`;   // direkt interpolation av heltal
