...
export default function setupPowerpointRoutes(app, db) {
  ...
  // Sök på fält eller "any" med paging och sortering
  ...
  // Hämta all metadata för ett PPT-dokument via id
  ...
  // Lista alla PPTs med paging och sortering
...
/*// backend/powerpointBackend/powerpoint-rest-routes.js
import { Router } from 'express';
...
export default function setupPowerpointRoutes(app, db) {
  ...
  // Sök på fält eller "any" med limit/offset och sortering
  router.get('/api/ppt-search/:field/:searchValue', requireDb, async (req, res) => {
    const { field, searchValue } = req.params;
    const like = `%${searchValue}%`;
    ...
    try {
      ...
      if (field === 'any') {
        ...
        sql = `
          ...
          LIMIT ? OFFSET ?
        `;
        params = Array(paths.length).fill(like).concat([limit, offset]);
      } else {
        if (!allowedFields[field]) {
          return res.status(400).json({ error: "Invalid field name" });
        }
  ...
  // Lista alla PPTs med limit/offset och sortering
  router.get('/api/ppt', requireDb, async (req, res) => {
    let limit = Number.parseInt(req.query.limit ?? '100', 10);
    let offset = Number.parseInt(req.query.offset ?? '0', 10);
    let sortField = req.query.sortField ?? 'title';
